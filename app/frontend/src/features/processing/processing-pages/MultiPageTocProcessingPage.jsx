...
const CARD_ID = "multipage-records";

export function MultiPageTocProcessingPage() {
  const {
    busyCards,
  ...
  const version = getDomainVersion("multipage-records");
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    processingFetch(
      "/processing/table/?card=multipage-records&offset=0&limit=1&includeFacets=0",
      { cache: "no-store" },
    )
      .then((payload) => {
        if (cancelled) {
  ...
  return (
    <PageFrame pageId="multipage" title="Multi-page TOC">
      <OverviewPanel
        pageId="multipage"
        loading={!summaryLoaded}
        stats={[
          { id: "total", label: "Multi-page books", value: summary.total || 0 },
      ...
      <div className="processing-card-grid">
        <ProcessingDataCard
          ...
          cardId="records"
          cardKey={CARD_ID}
          title="Multi-page TOC books"
          description="Books whose source table of contents spans multiple pages, with their creation status. Failed rows could not retrieve every TOC page (usually a sign-in/cookie issue)."
          className="processing-card-span-full processing-multipage-records-card"
          bookColumnMode="wide"
          showDetailsColumn
