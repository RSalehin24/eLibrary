...
> **⚠️ DEPRECATED:** This document has been superseded by [production-resource-configuration.md](production-resource-configuration.md).
> Please refer to the updated document for the most current resource configuration guidance.
...
Book processing is I/O-heavy (many HTTP fetches to the source site) but each task holds a
